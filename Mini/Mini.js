...
async function getMiniPuzzle(forDate) {
    return new Promise((resolve, reject) => {
        const apiUrl = `https://www.nytimes.com/svc/crosswords/v6/puzzle/mini/${formatDate(forDate)}.json`;
...
async function main() {
    ...
    if (args.length > 0) {
        for (let i = 0; i < args.length; i++) {
            ...
            if (arg === 'help' || arg === '--help' || arg === '-h') {
                console.log(chalk.blue('Mini Crossword -- CLI Daily Puzzle Game'));
                console.log(chalk.blue('Type "exit" or "quit" to exit the game'));
                console.log(chalk.blue('Type "help" to see this message again'));
                console.log(chalk.blue('Type "grid" to see the crossword grid'));
    ...
    try {
        puzzle = await getMiniPuzzle(date);
    } catch (err) {
...
async function playGame() {
    while (true) {
        ...
        if (parts.length < 2) {
            ...
            if (parts[0] === 'help') {
                console.log(chalk.blue('Enter clue number and answer, or use commands like "grid", "exit"'));
                continue;
            }
            if (parts[0] === 'grid') {
                displayPuzzleInfo();
