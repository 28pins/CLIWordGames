...
async function main() {
    ...
    consolePort.log(chalkPort.blue(`Mini puzzle for ${date.toDateString()}:\n`));
    ...
    while (guesses.length < MAX_GUESSES) {
        ...
        if (guess === wordOfDay) {
            ...
            consolePort.log(`Mini ${lookupDailyIndex(date)} ${guesses.length}/${MAX_GUESSES}${hardMode ? '*' : ''}\n`);
    ...
    consolePort.log(`Mini ${lookupDailyIndex(date)} X/${MAX_GUESSES}${hardMode ? '*' : ''}\n`);
}

function displayKeyboard(answerWord) {
    consolePort.log(hardMode ? chalkPort.yellow('HARD MODE') + chalkPort.blue(' -- Mini -- CLI Daily Word Game -- help for instructions\n') : chalkPort.blue('Mini -- CLI Daily Word Game -- help for instructions\n'));
...
function lookupDailyIndex(d = new Date()) { const epoch = new Date('2024-05-27T00:00:00'); return getDaysSince(epoch) - getDaysSince(d); }
