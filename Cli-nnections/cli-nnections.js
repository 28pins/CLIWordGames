...
function renderPuzzle(puzzle, pos, completed) {
    ...
    if (isImagePuzzle) {
        for (const group of puzzle.categories) {
            for (const card of group.cards) {
                if (card.image_url) {
                    const altTextLength = card.image_alt_text ? card.image_alt_text.length : 9; // '[Image]' length
                    if (altTextLength > maxLength) {
                        maxLength = altTextLength;
                    }
                }
            }
        }
            ...
            if (card) {
                ...
                if (card && content === '' && card.image_url) {//Means there is an image_url for today's puzzle: warn user and render alt text
                    if (isImagePuzzle) {
                        ...
                        isImagePuzzle = true;
                        console.log(chalk.red('Warning: This puzzle contains an image. Please view the puzzle on the website or press any arrow key to continue rendering the puzzle with alt text.'));
                        return;
                    }
                }
...
function printHelp() {
    helpLines.forEach(l => console.log(chalk.blue(l)));
}
/// MARK: - Main Game Loop
async function main() {
    ...
    let puzzle;
    try {
        puzzle = await getConnectionsPuzzle(date);
    } catch (error) {
